...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, identity) {
    ...
    router.on(wire.InstallMessage, function(channel, message) {
      ...
      // Only handle apk files
      log.info('Received install message:', message)
      if (pkg === undefined) {
        log.important('Will not process non-Android manifest')
        push.send([
          channel
        , reply.fail('Provided file is not an Android app')
        ])
        return
      }
    ...
    router.on(wire.UninstallMessage, function(channel, message) {
      ...
      // identify whether this is an Android
      if (identity.platform !== 'Android') {
        log.info('Not responding to non-Android app uninstall message')
        return
      }
