...
dbapi.startDeviceUsage = function(group, starts_from) {
  return db.run(r.table('users').get(group.email))
    .then(function(user) {
        return db.run(r.table('usageStats').insert({
...
dbapi.updateDeviceUsageLastSeen = function(group, last_seen) {
  return db.run(r.table('usageStats').filter({group: group.group, ends_at: null}).update({
    last_seen: last_seen
  }))
}

dbapi.updateDeviceUsage = function(group, ends_at) {
  return db.run(r.table('usageStats').filter({group: group.group, ends_at: null}).update({
    ends_at: ends_at
  }))
}
